...
Функция ВывестиРезультатОдногоЗапроса(ИмяЗапроса, РезультатЗапроса, Открыта, ПараметрыВыводаЗапроса, Иерархия, КоличествоСтрок, МассивШириныКолонок)
	...
	Если ТипЗнч(РезультатЗапроса) = Тип("ДеревоЗначений") Тогда
		КоличествоСтрок = РезультатЗапроса.Выгрузить().Количество();
	КонецЕсли;
...
Функция БлокКонфигурацииСеанса(ФильтрТекущегоСеанса, ШаблоныТегов)
	...
	Если Не ЗначениеЗаполнено(НайтиФайлы(КаталогСЛогФайлами)) Тогда
...
Функция КлючиХраненияЛичные() Экспорт
	
	Ключи = Новый Структура();
	Ключи.Вставить("КлючОбъекта", "КонсольЗапросов_ПроизвольноеЗаполнение");
	Ключи.Вставить("КлючНастроек", "ВременныйЗапрос");
	
	Возврат Ключи;
	
КонецФункции

Функция КлючиХраненияОбщие() Экспорт
	
	Ключи = Новый Структура();
	Ключи.Вставить("КлючОбъекта", "КонсольЗапросов_ХранимыеЗапросы");
	Ключи.Вставить("ИмяПользователя", "_АдминистраторКонсолиЗапросов");
	
	Возврат Ключи;
	
КонецФункции

Функция ДелимсяОбработкой() Экспорт
