...
// Возвращаемое значение:
//  Структура Из КлючИЗначение - Структура полей
Функция ПолучитьСтруктуруСообщенияКартинкиBase64(Знач Роль, Знач base64строка, Знач Промт) Экспорт

    СтруктураПолей = Новый Структура;
    МассивКонтента = Новый Массив;

	МассивКонтента.Добавить(Новый Структура("type,text", "input_text", Промт));
    МассивКонтента.Добавить(Новый Структура("type,image_url,detail", "input_image", base64строка, "auto"));
