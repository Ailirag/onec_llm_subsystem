...
Процедура ДиалогРасписанияРегламентногоЗаданияОткрытьЗавершение(Расписание, ДополнительныеПараметры) Экспорт
	Если Расписание <> Неопределено Тогда
		ВыделенныеСтроки = Элементы.СписокРегламентныхЗаданий.ВыделенныеСтроки;
		Если Элементы.СписокРегламентныхЗаданийИзменятьРасписаниеДляВыделенных.Пометка Тогда
			Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
				Строка = СписокРегламентныхЗаданий.НайтиПоИдентификатору(ВыделеннаяСтрока);
...
Процедура ВыполнитьЗаданиеВручнуюНаСервере(УникальныйИдентификатор)
	...
	КонецЗамера = ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоЗамера;
	
	Сообщить(СтрШаблон("Выполнено за %1 секунд (%2 миллисекунд)", Окр(КонецЗамера/1000, 2), КонецЗамера));
...
Процедура ВключитьВыбранные(Команда)
	ИзменитьПоложениеРегламентныхЗаданий(Истина);
КонецПроцедуры

&НаКлиенте
Процедура ВыключитьВыбранные(Команда)
	ИзменитьПоложениеРегламентныхЗаданий(Ложь);
КонецПроцедуры
...
Процедура ВывестиФоновые()
	...
	Для Каждого Фоновое Из Фоновые Цикл
		...
		НоваяСтрока.НачалоДата = Фоновое.Начало;
		НоваяСтрока.КонецДата = Фоновое.Конец;
	...
	СписокФоновыхЗаданий.Сортировать("НачалоДата");
...
&НаКлиенте
Процедура ИзменятьРасписаниеДляВыделенных(Команда)
	Элементы.СписокРегламентныхЗаданийИзменятьРасписаниеДляВыделенных.Пометка = НЕ Элементы.СписокРегламентныхЗаданийИзменятьРасписаниеДляВыделенных.Пометка;
КонецПроцедуры

&НаКлиенте
Процедура ИзменитьПоложениеРегламентныхЗаданий(Состояние)
	
	ВыделенныеСтроки = Элементы.СписокРегламентныхЗаданий.ВыделенныеСтроки;
	Для Каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		Строка = СписокРегламентныхЗаданий.НайтиПоИдентификатору(ВыделеннаяСтрока);
		Если ИзменитьСостояниеРегламентногоЗадания(Строка.Идентификатор, Строка.Наименование, Состояние) Тогда
			Строка.Использование = Состояние;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ИзменитьСостояниеРегламентногоЗадания(Идентификатор, ИмяЗадания, Состояние)
	ОбъектЗадания = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(Идентификатор);
	РедОбъектЗадания = ОбъектЗадания;
	
	РедОбъектЗадания.Использование = Состояние;
	Попытка
		РедОбъектЗадания.Записать();
		Возврат Истина;
	Исключение
		ВызватьИсключение "Произошла ошибка при сохранении состояния по заданию " + ИмяЗадания + "
		|Подробное описание ошибки: " + ОписаниеОшибки();
		Возврат Ложь;
	КонецПопытки;
