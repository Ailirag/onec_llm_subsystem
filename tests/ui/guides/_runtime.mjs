...
// Кнопку действия ищем «от противного»: ее подпись задается настройкой места
// запуска и может быть любой — «Создать заявку на расход», «Отправить на
// согласование». Искать по словам «Заполнить/Записать/Выполнить» значит
// сломаться на первой же осмысленной подписи, ради которой настройка и нужна.
// Поэтому отбрасываем постоянный состав мастера и вкладки карточки под ним.
async function кнопкаДействияМастера() {
	...
	const кандидаты = (состояние.buttons || [])
		.map(к => (typeof к === 'string' ? к : к.name))
		.filter(и => и && служебные.indexOf(и) < 0
			&& и.indexOf('Подменю') < 0 && и.indexOf('Группа') < 0 && и.indexOf('Форма') < 0);
	return кандидаты.length > 0 ? кандидаты[0] : undefined;
}
