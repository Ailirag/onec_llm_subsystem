...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СвойствоСуществует(ПараметрыСеанса, "ТекущийПользователь") Тогда
		ИмяПользователяПК = Строка(ПараметрыСеанса.ТекущийПользователь);
	Иначе
		Если ОбработкаОбъект.ДелимсяОбработкой() Тогда
			ОбъектСеть			= Новый COMОбъект("WScript.Network");
			ИмяПользователяПК	= ОбъектСеть.UserName;
		Иначе
			ИмяПользователяПК	= "_ПользовательОбработки";
		КонецЕсли;
	КонецЕсли;
	
	КлючиХранения = Новый Структура;
	КлючиХранения.Вставить("Личные", ОбработкаОбъект.КлючиХраненияЛичные());
	КлючиХранения.Вставить("Общие", ОбработкаОбъект.КлючиХраненияОбщие());
	
	Элементы.ДеревоОбъектовМетаданных.Видимость				= Элементы.ФормаМетаданные.Пометка;
	Элементы.ДеревоОбъектовМетаданныхПодробности.Видимость	= Элементы.ФормаМетаданные.Пометка;
	
	ОбработкаТаймаутаОчисткиХранилища();
...
Процедура ЗапуститьВыполнениеЗапроса(ВыводитьВременныеТаблицы)
	...
	Попытка
		Если ЗначениеЗаполнено(Объект.АдресСохранения) Тогда
			
			НачалоВыполнения = ТекущаяДата();
			ДД = СохранитьВыводВФайл();
			Сообщить(СтрШаблон("Файл сформирован за %1 сек.", ТекущаяДата() - НачалоВыполнения));
			
			НачалоВыполнения = ТекущаяДата();
			ДД.Записать(Объект.АдресСохранения);
			Сообщить(СтрШаблон("Файл записан по адресу %1 за %2 сек.", Объект.АдресСохранения, ТекущаяДата() - НачалоВыполнения));
			
			Объект.АдресСохранения = "";
			Элементы.ФормаСохранятьВыводВФайл.Пометка = Ложь;
		ИначеЕсли ВывестиВКонтекстLLM Тогда
			// Разделитель консоли пользователь мог настроить под себя: меняем его
			// только на время выгрузки для модели.
			ПрежнийРазделитель = Объект.РазделительФайла;
			Объект.РазделительФайла = ";";
			ДД = СохранитьВыводВФайл();
			Объект.РазделительФайла = ПрежнийРазделитель;
			ТекстРезультата = ПрочитатьТекстИзДвоичныхДанных(ДД);
			ДанныеЗапросаCSV = ТекстРезультата;
		Иначе
...
Функция ПрочитатьТекстИзДвоичныхДанных(ДвоичныеДанные, Кодировка = Неопределено) Экспорт
	...
	Исключение
		ВызватьИсключение "Не удалось прочитать текст из двоичных данных: " + ОписаниеОшибки();
...
&НаКлиенте
Процедура ЗаполнитьЗапросИзСтроки(Команда)
	...
	ЗаполнитьЗапросИзСтрокиНаСервере();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьЗапросИзСтрокиНаСервере(СтруктураВходящая = Неопределено)
	...
	//Проверка наличия сохраненных настроек
	Если СтруктураВходящая = Неопределено Тогда
		СохраненныйЗапрос		= ХранилищеОбщихНастроек.Загрузить(КлючиХранения.Личные.КлючОбъекта, КлючиХранения.Личные.КлючНастроек, , ИмяПользователяПК);
	Иначе
		СохраненныйЗапрос = СтруктураВходящая;
	КонецЕсли;
	
	Если СохраненныйЗапрос <> Неопределено Тогда
		СохраненныйТекст		= СохраненныйЗапрос.Текст;
		СохраненныеПараметры	= СохраненныйЗапрос.Параметры;
		ВремяСоздания			= СохраненныйЗапрос.ВремяСоздания;
	Иначе
		Если СтруктураВходящая = Неопределено Тогда
			Сообщить(СтрШаблон("Параметры не сохранены. Необходимо вызвать:
					|При наличии БСП:
					|Если Строка(ПараметрыСеанса.ТекущийПользователь) = ""Имя вашего пользователя"" Тогда
					|	ИмяПользователя		= ПараметрыСеанса.ТекущийПользователь;
					|	СтруктураХранения	= Новый Структура(""ВремяСоздания, Текст, Параметры"", ТекущаяДата(), Запрос.Текст, Запрос.Параметры);
					|	ХранилищеОбщихНастроек.Сохранить(""%1"", ""%2"", СтруктураХранения, , ИмяПользователя);
					|КонецЕсли;
					|
					|Без БСП:
					|ОбъектСеть			= Новый COMОбъект(""WScript.Network"");
					|ИмяПользователяПК	= ОбъектСеть.UserName;
					|Если Строка(ИмяПользователяПК) = ""Имя вашего пользователя ПК"" Тогда
					|	СтруктураХранения	= Новый Структура(""ВремяСоздания, Текст, Параметры"", ТекущаяДата(), Запрос.Текст, Запрос.Параметры);
					|	ХранилищеОбщихНастроек.Сохранить(""%1"", ""%2"", СтруктураХранения, , ИмяПользователяПК);
					|КонецЕсли;", 
					КлючиХранения.Личные.КлючОбъекта, КлючиХранения.Личные.КлючНастроек));
		Иначе
	...
	ТекущийЗапрос.Имя   = ПолучитьИмяЗапросаПоФорме(ЭтотОбъект, ТекущийЗапрос.Текст);
	ТекущийЗапрос.Текст = СохраненныйТекст;
	
	ТекстЗапроса.УстановитьТекст(ТекущийЗапрос.Текст);
	
	//
	
	ОбработатьДобавленныеПараметры(СохраненныеПараметры);
	
	Сообщить("Запрос загружен.");
	
КонецПроцедуры

&НаСервере
Процедура ОбработатьДобавленныеПараметры(СтруктураПараметров)
	...
	Для Каждого КлючИЗначение Из СтруктураПараметров Цикл
		...
		Иначе
			ПараметрЗапроса = Найденные[0];
		КонецЕсли;
		Если ТипЗнч(КлючИЗначение.Значение) = Тип("Массив") ИЛИ ТипЗнч(КлючИЗначение.Значение) = Тип("ФиксированныйМассив")
			...
			Если ТипЗнч(КлючИЗначение.Значение) = Тип("ФиксированныйМассив") Тогда
				МассивДляПодстановки = Новый Массив;
				Для Каждого ЭлементМассива Из КлючИЗначение.Значение Цикл
					МассивДляПодстановки.Добавить(ЭлементМассива);
				КонецЦикла;
			ИначеЕсли ТипЗнч(КлючИЗначение.Значение) = Тип("СписокЗначений") Тогда
				МассивДляПодстановки = Новый Массив;
				Для Каждого ЭлементСписка Из КлючИЗначение.Значение Цикл
					МассивДляПодстановки.Добавить(ЭлементСписка.Значение);
				КонецЦикла;
			Иначе
				МассивДляПодстановки = КлючИЗначение.Значение;
			КонецЕсли;
			
			Значение.ЗагрузитьЗначения(МассивДляПодстановки);
			ПараметрЗапроса.Тип = "СписокЗначений";
		Иначе
			Значение = КлючИЗначение.Значение;
			ПараметрЗапроса.Тип = ОбъектОбработки().ИмяТипаИзЗначения(Значение);
...
&НаКлиенте
Процедура ПараметрыИзСтроки(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьПараметрыИзСтруктурыСтроки", ЭтаФорма);
	
	ПоказатьВводСтроки(Оповещение, "", "Введите строку полученную от структуры параметров функцией ЗначениеВСтрокуВнутр()",, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПараметрыИзСтруктурыСтроки(Результат, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьИзСтруктурыПараметровНаСервере(Результат);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьИзСтруктурыПараметровНаСервере(СтрокаСтруктуры)
	
	Попытка
		ПараметрыИзСтроки = ЗначениеИзСтрокиВнутр(СтрокаСтруктуры);
		
		ОбработатьДобавленныеПараметры(ПараметрыИзСтроки);
	Исключение
		Сообщить("Ошибка преобразования значения из строки. " + ОписаниеОшибки());
	КонецПопытки;
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьПараметрСтрокой(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ЗаполнитьПараметрИзКода", ЭтаФорма);
	
	ОткрытьФорму("ВнешняяОбработка.КонсольЗапросов.Форма.ФормаДобавленияПараметраКодом",,,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
	//СтрокаКода = "// Доступна функция СН(СтрокаНавигации) - Получить ссылку по навигационной ссылке.
	//|ИмяПараметра = 
	//|Результат =";
	//
	//ПоказатьВводСтроки(Оповещение, СтрокаКода, "Введите произвольный код, заполните переменные ИмяПараметра, Результат",, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПараметрИзКода(Результат, ДополнительныеПараметры) Экспорт 
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Возврат;
	КонецЕсли;
	
	//Если СтрНайти(нрег(результат), "результат") = 0 Тогда
	//	Сообщить("Переменная ""Результат"" обязательна для инициализации.");
	//	Возврат;
	//КонецЕсли;
	
	ЗаполнитьПараметрИзКодаНаСервере(Результат);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПараметрИзКодаНаСервере(Структура)
	
	ИмяПараметра = Структура.ИмяПеременной;
	Результат = ПолучитьИзВременногоХранилища(Структура.Адрес);
	
	//Попытка
	//	Выполнить Строка;
	//Исключение
	//	Сообщить("Ошибка выполнения алгоритма. " + ОписаниеОшибки());
	//	Возврат;
	//КонецПопытки;
	//
	//Если ИмяПараметра = Неопределено Тогда
	//	ИмяПараметра = "Параметр_" + СтрЗаменить(Новый УникальныйИдентификатор, "-", "_");
	//КонецЕсли;
	
	Попытка
		Параметр = Новый Структура;
		Параметр.Вставить(ИмяПараметра, Результат);
	Исключение
		Сообщить("Ошибка создания структуры параметра. " + ОписаниеОшибки());
	КонецПопытки;
	
	ОбработатьДобавленныеПараметры(Параметр);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗамерВыполнения(Команда)
	
	Путь = Объект.ПутьКФормам + "." + "ФормаЗамераВремени";
		
	ПередаваемыеЗапросы = ПоместитьЗапросыВСтруктуру();
	ОткрытьФорму(Путь, ПередаваемыеЗапросы, ЭтотОбъект, Новый УникальныйИдентификатор);
...
Функция ПоместитьЗапросыВСтруктуру()
	...
	Для каждого СтрЗапросы Из Объект.Запросы Цикл
		Если СтрЗапросы.Идентификатор = ИдентификаторТекущегоЗапроса Тогда
			ИмяЗапроса = СокрЛП(СтрЗапросы.Имя);
			Прервать;
		КонецЕсли;
	КонецЦикла;	
	...
	ПараметрАдрес = Новый Структура;
	ПараметрАдрес.Вставить("АдресХранилища", АдресХранилища);
	ПараметрАдрес.Вставить("ИмяЗапроса", 	 ИмяЗапроса);
...
&НаКлиенте
Процедура СохранитьВХранилище(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПослеПодтвержденияСохраненияВХранилище", ЭтаФорма);
	ПоказатьВопрос(Оповещение, "Вы уверены что хотите сохранить данную настройку в память?", РежимДиалогаВопрос.ДаНет,,,"Подтвердите действие");
	
КонецПроцедуры

&НаКлиенте
Процедура СохранитьВХранилищеОбщее(Команда)
	
	Оповещение = Новый ОписаниеОповещения("ПослеВводаИмениЗапросаОбщее", ЭтаФорма);
	
	ПоказатьВводСтроки(Оповещение, "", "Введите псеводним запроса");
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаИмениЗапросаОбщее(ПсевдонимЗапроса, ДополнительныеСвойства) Экспорт
	
	ОчиститьСообщения();
	
	Если Не ЗначениеЗаполнено(ПсевдонимЗапроса) Тогда
		Возврат;
	КонецЕсли;
	
	Попытка
		СсылкаНаЗапрос = СохранитьЗапросВОбщееХранилище(ПсевдонимЗапроса);
		Сообщить("Запрос сохранен. Ссылка:");
		Сообщить(СсылкаНаЗапрос);
	Исключение
		Сообщить("Ошибка при сохранении в общее хранилище. " + ОписаниеОшибки());
	КонецПопытки;
	
КонецПроцедуры

&НаСервере
Функция СохранитьЗапросВОбщееХранилище(ПсевдонимЗапроса)
	
	СтруктураХранения = ПолучитьСтруктуруТекущегоЗапроса(ПсевдонимЗапроса);
	ИмяХранимогоЗапроса = СтрШаблон("%1_%2", ИмяПользователяПК, Новый УникальныйИдентификатор);
	
	ХранилищеОбщихНастроек.Сохранить(КлючиХранения.Общие.КлючОбъекта, ИмяХранимогоЗапроса, СтруктураХранения, , КлючиХранения.Общие.ИмяПользователя);
	
	Возврат ИмяХранимогоЗапроса;
	
КонецФункции

&НаКлиенте
Процедура ПослеПодтвержденияСохраненияВХранилище(Результат, ДополнительныеПараметры) Экспорт
...
Процедура СохранитьВХранилищеНаСервере()
	...
	СтруктураХранения = ПолучитьСтруктуруТекущегоЗапроса();
	ХранилищеОбщихНастроек.Сохранить(КлючиХранения.Личные.КлючОбъекта, КлючиХранения.Личные.КлючНастроек, СтруктураХранения, , ИмяПользователяПК);
	Сообщить("Запрос сохранен в хранилище");
	
КонецПроцедуры

&НаСервере
Функция ПолучитьСтруктуруТекущегоЗапроса(ПсевдонимЗапроса = "")
	
	Найденный = Объект.Запросы.НайтиСтроки(Новый Структура("Идентификатор", ИдентификаторТекущегоЗапроса))[0];
	...
	Для Каждого ПараметрЗапроса Из ПараметрыЗапроса Цикл
		...
		Если ТипЗнч(ПараметрЗапроса.ЗначениеВФорме) = Тип("СписокЗначений") Тогда
			СохраняемоеЗначение = ПараметрЗапроса.ЗначениеВФорме.ВыгрузитьЗначения();
		Иначе
			СохраняемоеЗначение = ПараметрЗапроса.ЗначениеВФорме;
		КонецЕсли;
		
		СохраняемыеПараметры.Вставить(ПараметрЗапроса.Имя, СохраняемоеЗначение);
		
	КонецЦикла;
	
	СтруктураХранения = Новый Структура("ВремяСоздания, Текст, Параметры", ТекущаяДата(), Найденный.Текст, СохраняемыеПараметры);
	
	Если ЗначениеЗаполнено(ПсевдонимЗапроса) Тогда
		СтруктураХранения.Вставить("ИмяЗапроса", ПсевдонимЗапроса); 
	КонецЕсли;
...
&НаСервере
Процедура ОбработкаТаймаутаОчисткиХранилища()
	
	ХранимыйЗапрос = ХранилищеОбщихНастроек.Загрузить(КлючиХранения.Личные.КлючОбъекта, КлючиХранения.Личные.КлючНастроек, , ИмяПользователяПК);
	Если ХранимыйЗапрос = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Таймаут = 129600; // 1.5 дня
	
	Если ХранимыйЗапрос.ВремяСоздания + Таймаут < ТекущаяДата() Тогда
		ХранилищеОбщихНастроек.Удалить(КлючиХранения.Личные.КлючОбъекта, КлючиХранения.Личные.КлючНастроек, ИмяПользователяПК);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнениеПодсказка(Команда)
	
	ПодсказкаЗаполнения = СтрШаблон(
	"Для удобства отладки запроса предусмотрен следующий мезанизм:
	|В исследуемую базу ставится расширение, перед выполнением сформированого запроса (перед конструкцией Запрос.Выполнить()) 
	|необходимо вставить следующую конструкцию, после чего позволить запросу сформироваться в режиме предприятия.
	|	
	|При наличии БСП:
	|Если Строка(ПараметрыСеанса.ТекущийПользователь) = ""Имя вашего пользователя"" Тогда
	|	ИмяПользователя		= ПараметрыСеанса.ТекущийПользователь;
	...
	|КонецЕсли;
	|	
	|Далее в данной консоли запросов, выбрать пункт меню ""Заполнение"" => ""Запрос из хранилища"".
	|Для предотвращения замусоренности хранится всего один запрос. Время жизни запроса 1.5 дня, после этого
	|при запуске обработки хранилище будет очищено.", КлючиХранения.Личные.КлючОбъекта, КлючиХранения.Личные.КлючНастроек);
	
	ПоказатьВводСтроки(Новый ОписаниеОповещения, ПодсказкаЗаполнения, "Подсказка", , Истина);
...
&НаКлиенте
Процедура ЗагрузитьЗапросПоСсылке(Команда)
	
	Оповещение = Новый ОписаниеОповещения("ПослеВводаИмениСсылкиЗапроса", ЭтаФорма);
	
	ПоказатьВводСтроки(Оповещение, "", "Введите ссылку запроса");
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаИмениСсылкиЗапроса(СсылкаНаЗапрос, ДополнительныеПараметры) Экспорт
	
	ОчиститьСообщения();
	
	Если Не ЗначениеЗаполнено(СсылкаНаЗапрос) Тогда
		Возврат;
	КонецЕсли;
	
	ЗаполнитьЗапросИзХранилищаПоСсылке(СсылкаНаЗапрос);
...
Процедура ЗаполнитьЗапросИзХранилищаПоСсылке(СсылкаНаЗапрос)
	...
	СтруктураЗапроса = ХранилищеОбщихНастроек.Загрузить(КлючиХранения.Общие.КлючОбъекта, СсылкаНаЗапрос, , КлючиХранения.Общие.ИмяПользователя);
	...
	ЗаполнитьЗапросИзСтрокиНаСервере(СтруктураЗапроса);
	
КонецПроцедуры

&НаКлиенте
Процедура ХранимыеЗапросыОткрытьФормуСохраненныхЗапросов(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ИмяПользователяПк", ИмяПользователяПК);
	ПараметрыОткрытия.Вставить("ТекущийЗапрос", ПолучитьСтруктуруТекущегоЗапроса());
	
	Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияФормыХранимыхЗапросов", ЭтаФорма);
	
	Путь = Объект.ПутьКФормам + "." + "ФормаХранилищаЗапросов";
	ОткрытьФорму(Путь, ПараметрыОткрытия, ЭтаФорма,,,,Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗакрытияФормыХранимыхЗапросов(РезультатЗакрытия, ДополнительныеПараметры) Экспорт 
	
	ОчиститьСообщения();
	
	Если ЗначениеЗаполнено(РезультатЗакрытия) Тогда
		ЗаполнитьЗапросИзСтрокиНаСервере(РезультатЗакрытия);
	КонецЕсли;
...
&НаКлиенте
Процедура СохранятьВыводВФайл(Команда)
	Элементы.ФормаСохранятьВыводВФайл.Пометка = Не Элементы.ФормаСохранятьВыводВФайл.Пометка;
	
	Если Элементы.ФормаСохранятьВыводВФайл.Пометка Тогда
		Оповещение = Новый ОписаниеОповещения("ПослеВыбораРасположенияВывода", ЭтаФорма);
		ДиалогВыбора = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Сохранение);
		ДиалогВыбора.Фильтр = "Табличный файл (*.csv)|*.csv";
...
&НаКлиенте
Процедура ПослеВыбораРасположенияВывода(Результат, ДополнительныеПараметры) Экспорт
	
	Если ЗначениеЗаполнено(Результат) Тогда
		Объект.АдресСохранения = Результат[0];
		Оповещение = Новый ОписаниеОповещения("ПослеВводаРазделителяCSV", ЭтаФорма);
		ПоказатьВводСтроки(Оповещение, ";", "Введите разделитель CSV", 10);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПослеВводаРазделителяCSV(Результат, ДополнительныеПараметры) Экспорт
	
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Результат = ";";
	КонецЕсли;
	
	Объект.РазделительФайла = Результат;
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузкаПараметровВЗапрос(Запрос, ПараметрыЗапроса) Экспорт
...
Функция СохранитьВыводВФайл()
	
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса.ПолучитьТекст();
	ЗагрузкаПараметровВЗапрос(Запрос, Объект.Параметры);
	...
	ВыполняемыйКод = Новый Массив;
	
	МассивЗаголовков = Новый Массив;
	
	
	ВыполняемыйКод.Добавить("ВыгрузкаРезультата = Результат.Выбрать();");
	ВыполняемыйКод.Добавить("Пока ВыгрузкаРезультата.Следующий() Цикл");
	
	ЗаполнениеКолонок = Новый Массив;
	
	Для Каждого Колонка Из Результат.Колонки Цикл
		МассивЗаголовков.Добавить(Колонка.Имя);
		ЗаполнениеКолонок.Добавить(СтрШаблон("Строка(ВыгрузкаРезультата[""%1""])", Колонка.Имя));
	КонецЦикла;
	
	Текст.ЗаписатьСтроку(СтрСоединить(МассивЗаголовков, Объект.РазделительФайла));
	
	СтрокаЗаписи = "ТекстЗаписи = ";
	ПерваяЗапись = Истина;
	Для Каждого Заполнение Из ЗаполнениеКолонок Цикл
		Если ПерваяЗапись Тогда
			СтрокаЗаписи = СтрокаЗаписи + Заполнение;
			ПерваяЗапись = Ложь;
		Иначе
			СтрокаЗаписи = СтрокаЗаписи + "+""" + Объект.РазделительФайла  + """+" + Заполнение;
		КонецЕсли;
	КонецЦикла;
	
	ВыполняемыйКод.Добавить(СтрокаЗаписи + ";");
	
	ВыполняемыйКод.Добавить("Текст.ЗаписатьСтроку(ТекстЗаписи);");
	ВыполняемыйКод.Добавить("КонецЦикла;");
	
	Выполнить СтрСоединить(ВыполняемыйКод, Символы.ПС);
...
&НаКлиенте
Процедура ВыгрузитьПараметрыВСтроку(Команда)
	СтруктураПараметров = Новый Структура();
	Для Каждого Параметр Из Объект.Параметры Цикл
		СтруктураПараметров.Вставить(Параметр.Имя, Параметр.ЗначениеВФорме);
	КонецЦикла;
	
	ПоказатьВводСтроки(Новый ОписаниеОповещения, ЗначениеВСтрокуСервер(СтруктураПараметров), "Параметры", , Истина);
...
Процедура КонсольКода_ДеревоМетаданныхНаСервере()
	...
	Элементы.ФормаМетаданные.Пометка = НЕ Элементы.ФормаМетаданные.Пометка;
	Если Элементы.ФормаМетаданные.Пометка Тогда
		Элементы.ФормаМетаданные.ЦветФона = WebЦвета.НебесноГолубой;
	Иначе
		Элементы.ФормаМетаданные.ЦветФона = ЦветаСтиля.ЦветФонаКнопки;
	КонецЕсли;
...
Процедура ДеревоОбъектовМетаданныхПодробностиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//КонецЕсли;
	//
	//ИмяРеквизита = СтрРазделить(ТекущиеДанные.Представление, " ", Ложь)[0];
	//
	//ОбъектКопирования = Новый COMОбъект("htmlfile"); 
	//ОбъектКопирования.ParentWindow.ClipboardData.SetData("Text", ИмяРеквизита);
...
Процедура ДеревоОбъектовМетаданныхВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	//КонецЕсли;
	//
	//ИмяРодителя = СтрРазделить(Родитель.Представление, " ", Ложь)[0];
	//
	//ИмяРеквизита = СтрРазделить(ТекущиеДанные.Представление, " ", Ложь)[0];
	//
	//ОбъектКопирования = Новый COMОбъект("htmlfile"); 
	//ОбъектКопирования.ParentWindow.ClipboardData.SetData("Text", ИмяРодителя + "." + ИмяРеквизита);
КонецПроцедуры

&НаКлиенте
Процедура ОбогатитьДаннымиЗапроса(Команда)
	// Флаг действует на один запуск: иначе обычное «Выполнить» тоже ушло бы
	// в данные для модели, а результат запроса не показался бы.
	ВывестиВКонтекстLLM = Истина;
	ЗапуститьВыполнениеЗапроса(Ложь);
	ВывестиВКонтекстLLM = Ложь;
	КоличествоСимволовДанных = СтрДлина(ДанныеЗапросаCSV);
КонецПроцедуры

&НаСервере
Процедура ОбогатитьДаннымиЗапросаНаСервере()
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьЗапросLLM(Команда)

	Если Не ЗначениеЗаполнено(Промт) Тогда
		Возврат;
	КонецЕсли;
	НовоеСообщение = Промт;
	ОтправитьСообщение(Команда);
...
Процедура ОтправитьСообщение(Команда)
	...
	ТекстСообщения = СокрЛП(НовоеСообщение);
	Если Не ЗначениеЗаполнено(ТекстСообщения) Тогда
		Сообщить("Введите сообщение.");
		Возврат;
	КонецЕсли;

	Промт = ТекстСообщения;
	НовоеСообщение = "";
	СтрокаДиалога = СообщенияДиалога.Добавить();
	СтрокаДиалога.Время = ТекущаяДата();
	СтрокаДиалога.Роль = "Вы";
	СтрокаДиалога.Сообщение = ТекстСообщения;

	ВыполнитьЗапросLLMНаСервере();
	Промт = "";
...
Функция ПроверитьНастройкиДиалога()
	...
	Если Не ЗначениеЗаполнено(Провайдер)
			...
			Или Не ЗначениеЗаполнено(СистемныйПромт)
			Или Не ЗначениеЗаполнено(ТипAPI) Тогда
		Сообщить("Заполните провайдера, модель, тип API и системную инструкцию.");
		Возврат Ложь;
	КонецЕсли;
	Возврат Истина;
...
&НаСервере
Процедура ВыполнитьЗапросLLMНаСервере()
	...
	СтруктураКонтекста.ДанныеКонтекста = ДанныеЗапросаCSV;
	СтруктураКонтекста.Наименование = "Данные запроса из базы";
	СтруктураКонтекста.ИдентификаторДляЛогики = "ДанныеЗапросаИзБазы";
		
	ДополнительныйКонтекст.Добавить(СтруктураКонтекста);
	
	Клиент = Обработки.AI_Клиент.Создать();
	Клиент.Инициализировать(Провайдер, Модель, ТипAPI, РежимДиалога, ГлубинаДиалога);
	Клиент.ИнициализироватьСистемныйПромтПроизвольный(СистемныйПромт, 0.3, 50000, Неопределено, Неопределено, ДополнительныйКонтекст, Агент);
	
	Если ЗначениеЗаполнено(ИдентификаторДиалогаResponseApi) Тогда
		Клиент.УстановитьПредыдущийIDОтвета(ИдентификаторДиалогаResponseApi);
	КонецЕсли;
	...
	Если РежимДиалога И ЗначениеЗаполнено(ИдентификаторДиалога)
		...
		Клиент.ИдентификаторДиалога = ИдентификаторДиалога;
	КонецЕсли;
	...
	ШагиАгента.Очистить();
	Для Каждого ШагАгента Из Клиент.ШагиАгента Цикл
		СтрокаШага = ШагиАгента.Добавить();
		СтрокаШага.НомерШага = ШагАгента.НомерШага;
		СтрокаШага.Инструмент = ШагАгента.Инструмент;
		СтрокаШага.Статус = ШагАгента.Статус;
		СтрокаШага.ДлительностьМс = ШагАгента.ДлительностьМс;
		СтрокаШага.Запрос = ШагАгента.Запрос;
		СтрокаШага.Результат = ШагАгента.Результат;
	КонецЦикла;
	
	// ── Обработка результата ─────────────────────────────────────────────
	Если ЗначениеЗаполнено(Клиент.ОшибкаОбработки) Тогда
		Сообщить("Ошибка обработки: " + Клиент.ОшибкаОбработки);
	КонецЕсли;
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		Если ТипЗнч(Клиент.Ответ) = Тип("Строка") Тогда
			НачалоОтвета = Клиент.Ответ;
		Иначе
			НачалоОтвета = "";
		КонецЕсли;
		ТекстОтвета = НачалоОтвета + Символы.ПС + ОписаниеОшибки();
	КонецПопытки;
	Если Не ЗначениеЗаполнено(ТекстОтвета)
			И ЗначениеЗаполнено(Клиент.ОшибкаОбработки) Тогда
		ТекстОтвета = "Ошибка: " + Клиент.ОшибкаОбработки;
	КонецЕсли;
	ТекстОтвета = Строка(ТекстОтвета);
	
	ОтветНейросети.УстановитьТекст(ТекстОтвета);
	СтрокаДиалога = СообщенияДиалога.Добавить();
	СтрокаДиалога.Время = ТекущаяДата();
	СтрокаДиалога.Роль = ?(ЗначениеЗаполнено(Агент), Строка(Агент), "Модель");
	СтрокаДиалога.Сообщение = ТекстОтвета;
	...
	Если ТипAPI = Перечисления.AI_ТипAPI.Responses Тогда
		НовыйID = Клиент.ИдентификаторДиалога;
		Если ЗначениеЗаполнено(НовыйID) Тогда
			ИдентификаторДиалогаResponseApi = НовыйID;
		КонецЕсли;
	Иначе
		// ChatCompletions: ID диалога — UUID из AI_Диалоги
		Если РежимДиалога И ЗначениеЗаполнено(Клиент.ИдентификаторДиалога) Тогда
			ИдентификаторДиалога = Клиент.ИдентификаторДиалога;
		КонецЕсли;
	КонецЕсли;
	
	// Статистика токенов
	ВходящиеТокены = Клиент.ВходящиеТокены;
	ВыходящиеТокены = Клиент.ВыходящиеТокены;
	ТокеныНавыков = Клиент.ТулТокены;
...
Процедура СброситьДиалог(Команда)
	...
	ОчиститьСообщения();
	СообщенияДиалога.Очистить();
	ШагиАгента.Очистить();
	НовоеСообщение = "";
	Промт = "";
	ОтветНейросети.УстановитьТекст("");
	...
	Сообщить("Идентификатор диалога сброшен");
	
КонецПроцедуры

&НаКлиенте
Процедура ДанныеЗапросаCSVПриИзменении(Элемент)
	КоличествоСимволовДанных = СтрДлина(ДанныеЗапросаCSV);
КонецПроцедуры
